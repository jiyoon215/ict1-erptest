	...
	var conf={url:'/levelinfo',
			  success:function(res){
				...
				for(var li of res){
					...
					html += '<td>'+ li.linum +'</td>';
					html += '<td id="lilevel' + li.linum + '">'+li.lilevel+'</td>';
					html += '<td id="liname' + li.linum + '">'+li.liname+'</td>';
					html += '<td id="lidesc' + li.linum + '">'+li.lidesc+'</td>';
...
<body>
	...
	<button onclick="location.href='/url/levelinfo:insert2'">추가</button>
	
	<script>
	
	function updateLevel(linum){
		/*  // 기존td를 input으로 변경하는 방법
		var changetd=document.querySelectorAll('td[data-type]');
		for(var td of changetd){
		var type=td.getAttribute('data-type');
		var tdvalue=td.innerHTML;		
		td.innerHTML='<input type="'+type+'" value="'+tdvalue+'">';
		}  */
		
		var lilevel=document.querySelector('#lilevel'+linum);
		var liname=document.querySelector('#liname'+linum);
		var lidesc=document.querySelector('#lidesc'+linum);
		
		lilevel.innerHTML = '<input type="text" value="' + lilevel.innerHTML + '">';
		liname.innerHTML='<input type="text" value="' + liname.innerHTML + '">';
		lidesc.innerHTML='<input type="text" value="' + lidesc.innerHTML + '">';
